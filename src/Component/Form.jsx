...
export default function PropertyList() {
  ...
  // تحديث دور المستخدم إلى Landlord في Firebase Realtime Database
  async function updateUserRole(uid) {
    if (!uid) return;
    const db = getDatabase();
    try {
      await update(ref(db, `users/${uid}`), { role: "landlord" });
      console.log("✅ User role updated to landlord");
    } catch (error) {
  ...
  async function sendDataToFirebase() {
    ...
    try {
      ...
      await updateUserRole(userId); // تحديث دور المستخدم بعد نجاح الإدخال
    } catch (error) {
